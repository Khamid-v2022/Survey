...
class AdminDistributieController extends MyController
{
    //
    public function index(){
        ...
        $forms = Webform::select('webforms.*', 'users.first_name', 'users.last_name', 'users.role', 'company.name AS company_name')
            ...
            ->orderBy('form_name', 'asc')->get();

        // get Tranees for our company
        ...
        if(count($our_trainee) > 0){
            for($i == 0; $i < count($our_trainee); $i++){
                ...
                $our_trainee[$i]['company_name'] = $company['name'];
            }
        }

        return view('adminDistributie', [
            'title' => $title,
            'user' => $this->user,
